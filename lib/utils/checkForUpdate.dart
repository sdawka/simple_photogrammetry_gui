...
Future<String?> checkForUpdates(String currentTag) async {
  final url = Uri.parse(
    'https://api.github.com/repos/edin45/simple_photogrammetry_gui/releases/latest',
  );

  try {
    final response = await http.get(url, headers: {'User-Agent': 'simple_photogrammetry_gui'});
