...
void main() {
  ...
  group('paths containing spaces', () {
    ...
    setUpAll(() async {
      fixtureDirectory = await Directory.systemTemp.createTemp('spg-fixture-');
      applicationSupportDirectory = Directory('${fixtureDirectory.path}/application-support');
      await applicationSupportDirectory.create();
      argumentLog = File('${applicationSupportDirectory.path}/colmap-arguments');
...
''');
    ...
    const pathCases = [
      ...
      _PathCase('spaced image and output paths', 'image folder', 'output folder'),
    ];

    for (final pathCase in pathCases) {
      test('prepares the pipeline workspace with ${pathCase.description}', () async {
        final root = await Directory.systemTemp.createTemp('spg-path-test-');
        addTearDown(() => root.delete(recursive: true));
        final images = Directory('${root.path}/${pathCase.imageDirectory}');
        final output = Directory('${root.path}/${pathCase.outputDirectory}');
        await images.create();
        await output.create();
        ...
        ).scanningProcess(view, images.path, output.path, 0, false);

        expect(view.status, isEmpty);
        expect(Directory('${output.path}/temp/sparse').existsSync(), isTrue);
        expect(Directory('${output.path}/temp/dense/sparse').existsSync(), isTrue);
        expect(File('${output.path}/temp/database.db').existsSync(), isTrue);
        expect(
